...
* character significance and case sensitivity are supported for external identifiers.
*/
#define MCU_MC_RGM_AR_RELEASE_MINOR_VERSION_C       3
/**
* @violates @ref Mcu_MC_RGM_c_REF_3 The compiler/linker shall be checked to ensure that 31
* character significance and case sensitivity are supported for external identifiers.
*/
#define MCU_MC_RGM_AR_RELEASE_REVISION_VERSION_C    1
#define MCU_MC_RGM_SW_MAJOR_VERSION_C               1
#define MCU_MC_RGM_SW_MINOR_VERSION_C               0
#define MCU_MC_RGM_SW_PATCH_VERSION_C               1
...
static FUNC( void, MCU_CODE ) Mcu_MC_RGM_ClearFesResetFlags(VAR( uint32, AUTOMATIC) u32RegValue)
{
    ...
    if ((uint32)0x00U != u32RegValue)
    {
        ...
        if ( (uint32)0x00U == u32TimeOut)
        {
            ...
            {
                /** @violates @ref Mcu_MC_RGM_c_REF_8 The returning shall be tested if error. **/
...
#if (STD_ON == MCU_RGM_REG_PROT_AVAILABLE)
/**
...
FUNC( void, MCU_CODE) Mcu_MC_RGM_SetUserAccessAllowed(void)
{
    /** @violates @ref Mcu_MC_RGM_c_REF_4 Conversion from int to pointer */
    /** @violates @ref Mcu_MC_RGM_c_REF_7 The cast is used to access memory mapped registers.*/
    SET_USER_ACCESS_ALLOWED(MC_RGM_BASEADDR, MC_RGM_PROT_MEM_U32);
}
#endif /* (STD_ON == MCU_RGM_REG_PROT_AVAILABLE) */
#endif /* (STD_ON == MCU_ENABLE_USER_MODE_SUPPORT) */
#endif /* MCU_ENABLE_USER_MODE_SUPPORT */

#ifdef MCU_DISABLE_RGM_INIT
...
{
    /* Temporary variable for DES register value */
    VAR( uint32, AUTOMATIC) u32DesResetReason = 0U;
    ...
    /** @violates @ref Mcu_MC_RGM_c_REF_4 Conversion from int to pointer */
    /** @violates @ref Mcu_MC_RGM_c_REF_7 The cast is used to access memory mapped registers.*/
    u32DesResetReason = (uint32)REG_READ32(MC_RGM_DES_ADDR32) & MC_RGM_DES_RWBITS_MASK32;
    ...
    if (MC_RGM_DES_F_POR_MASK32 == (u32DesResetReason & MC_RGM_DES_F_POR_MASK32))
    {
...
#else
    /* Enable (reconfigure) FRET only on "Power On Reset" or "Destructive Reset" */
    if (0x0U != u32DesResetReason)
    {
        /* 'Functional' Reset Escalation Threshold Register (MC_RGM_FRET) */
        /** @violates @ref Mcu_MC_RGM_c_REF_4 Conversion from int to pointer */
