...
/** @violates @ref Mcu_SSCM_c_REF_3 MISRA 2004 Rule 1.4, The compiler/linker shall be checked to ensure that 31 character
*   significance and case sensitivity are supported for external identifiers. */
#define MCU_SSCM_AR_RELEASE_MINOR_VERSION_C         3
/** @violates @ref Mcu_SSCM_c_REF_3 MISRA 2004 Rule 1.4, The compiler/linker shall be checked to ensure that 31 character
*   significance and case sensitivity are supported for external identifiers. */
#define MCU_SSCM_AR_RELEASE_REVISION_VERSION_C      1
#define MCU_SSCM_SW_MAJOR_VERSION_C                 1
#define MCU_SSCM_SW_MINOR_VERSION_C                 0
#define MCU_SSCM_SW_PATCH_VERSION_C                 1
...
#ifdef MCU_SSCM_REG_PROT_AVAILABLE
#if (STD_ON == MCU_SSCM_REG_PROT_AVAILABLE)
/**
...
{
    /** @violates @ref Mcu_SSCM_c_REF_4 Conversion from pointer to integer */
    /** @violates @ref Mcu_SSCM_c_REF_6 The cast is used to access memory mapped registers.*/
    SET_USER_ACCESS_ALLOWED(SSCM_BASEADDR, SSCM_PROT_MEM_U32);
}
#endif /* (STD_ON == MCU_SSCM_REG_PROT_AVAILABLE) */
#endif /* MCU_SSCM_REG_PROT_AVAILABLE */
...
FUNC( void, MCU_CODE) Mcu_SSCM_Init(P2CONST( Mcu_SSCM_ConfigType, AUTOMATIC, MCU_APPL_CONST) SSCM_pConfigPtr)
{
    ...
    if ( SSCM_STATUS_CER_MASK16 == (SSCM_STATUS_CER_MASK16 & u16State) )
    {
        ...
        {
            /** @violates @ref Mcu_SSCM_c_REF_7 The returning shall be tested if error. **/
            Dem_SetEventStatus((Dem_EventIdType)Mcu_pDemCfgPtr->Mcu_E_SscmCerFailureCfg.id, DEM_EVENT_STATUS_FAILED);
        }
#endif
    }
