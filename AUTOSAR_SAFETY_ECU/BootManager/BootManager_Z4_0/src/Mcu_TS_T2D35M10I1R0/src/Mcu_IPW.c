...
* not be implicitly converted to a different underlying type. Integer to pointer conversion, pecific
...
#include "Mcu_STCU.h"
#include "Mcu_SSCM.h"
#if (MCU_DISABLE_RAM_WS_CONFIG == STD_OFF)
  #include "Mcu_PRAM.h"
#endif /* (MCU_DISABLE_RAM_WS_CONFIG == STD_OFF) */
...
/**
* @brief    Detected production errors shall be reported to the Dem_SetEventStatus service of the
*           Diagnostic Event Manager (DEM) if the pre-processor switch McuDisableDemReportErrorStatus is set
*           The detection of production errors is configurable (ON / OFF) at precompile time.
*           The switch McuDisableDemReportErrorStatus shall activate or deactivate the detection
*           of all production errors.
*           Dem_SetEventStatus - Service to report production errors.
*
*/
...
#ifndef DISABLE_MCAL_INTERMODULE_ASR_CHECK
    #if ((MCU_IPW_AR_RELEASE_MAJOR_VERSION_C != DEM_AR_RELEASE_MAJOR_VERSION) || \
         (MCU_IPW_AR_RELEASE_MINOR_VERSION_C != DEM_AR_RELEASE_MINOR_VERSION) \
        )
        #error "AutoSar Version Numbers of Mcu_IPW.c and Dem.h are different"
    #endif
#endif
#endif
...
static FUNC( uint32, MCU_CODE) Mcu_Ipw_PrepareClkSourcesToBeConfigured (P2CONST(Mcu_ClockConfigType, AUTOMATIC, MCU_APPL_CONST) Mcu_pClockConfigPtr);
    ...
    static FUNC(Std_ReturnType, MCU_CODE) Mcu_Ipw_WaitForStableFXOSC(VAR(uint8, AUTOMATIC) u8ClockSourcesControl);
  #endif
#endif
...
/**
* @brief            Wait until FXOSC provides a stable clock or timeout expires, whichever comes first.
* @details          If enabled, raises DEM error (MCU_E_TIMEOUT_FAILURE) when timeout expires.
*                   Called by:
*                       - Mcu_Ipw_InitClock().
*
* @param[in]        u8ClockSourcesControl   Clock sources and PLLs under mcu control
*
...
static FUNC(Std_ReturnType, MCU_CODE) Mcu_Ipw_WaitForStableFXOSC(VAR(uint8, AUTOMATIC) u8ClockSourcesControl)
{
    VAR (uint32, AUTOMATIC) u32XoscStatus = 0U;
    VAR (uint32, AUTOMATIC) u32StabilizingTimeout = MCU_TIMEOUT_LOOPS;
    VAR(Std_ReturnType, AUTOMATIC) CheckStatus = (Std_ReturnType)E_OK;

    /* Check for FXOSC */
    if (MCU_IPW_FXOSC_UNDER_MCU_CONTROL == (u8ClockSourcesControl & MCU_IPW_FXOSC_UNDER_MCU_CONTROL))
    {
        /* Wait for FXOSC to stabilize. Otherwise, CMU may throw a spurious interrupt. */
#if (IPV_MC == IPV_MC_05_00_00_07)
        do
        {
            u32StabilizingTimeout--;
            u32XoscStatus = Call_Mcu_MC_ME_GetGlobalStatus() & MC_ME_GS_S_FXOSC_MASK32;
        } while ( 
                    (u32StabilizingTimeout > (uint32)0x0U) &&
                    (MC_ME_GS_S_FXOSC_MASK32 != u32XoscStatus)
                );
#else
        do
        {
            u32StabilizingTimeout--;
            u32XoscStatus = Call_Mcu_MC_ME_GetGlobalStatus() & MC_ME_GS_S_XOSC_MASK32;
        } while ( 
                    (u32StabilizingTimeout > (uint32)0x0U) &&
                    (MC_ME_GS_S_XOSC_MASK32 != u32XoscStatus)
                );
#endif /* (IPV_MC == IPV_MC_05_00_00_07) */
    }

    /* Check for timeout. */
    if ((uint32)0x0U == u32StabilizingTimeout)
    {
        ...
        {
            /** @violates @ref Mcu_IPW_c_REF_7 The returning shall be tested if error. **/
...
#if (MCU_DISABLE_RGM_INIT == STD_OFF)
...
#endif /*MCU_DISABLE_RGM_INIT == STD_OFF*/
...
FUNC( void, MCU_CODE) Mcu_Ipw_InitClock(P2CONST(Mcu_ClockConfigType, AUTOMATIC, MCU_APPL_CONST) Mcu_pClockConfigPtr)
{
       ...
       ) 
    {
        ...
        {
            Mcu_CMU_CSR_RCDIV_Config(Mcu_pClockConfigPtr->Mcu_CMU_pClockConfig);
        }
    }
       ...
       ) 
    {
        ...
        {
#if (MCU_FXOSC_UNDER_MCU_CONTROL == STD_ON)
            if ((Std_ReturnType)E_OK == Mcu_Ipw_WaitForStableFXOSC(Mcu_pClockConfigPtr->u8ClockSourcesControl))
            {
#endif
...
FUNC( Mcu_ResetType, MCU_CODE) Mcu_Ipw_GetResetReason(VAR(void, AUTOMATIC))
{
    ...
    /** @violates @ref Mcu_IPW_c_REF_8 MISRA 2004 Required Rule 10.1, Conversion from integer to pointer */
