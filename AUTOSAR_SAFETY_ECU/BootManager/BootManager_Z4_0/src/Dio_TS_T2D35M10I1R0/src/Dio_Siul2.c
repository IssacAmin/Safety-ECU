...
/* @violates @ref Dio_Siul2_C_REF_3 MISRA 2004 Required Rule 1.4, The used compilers use more than 31 chars for identifiers. */
#define DIO_SIUL2_AR_RELEASE_MINOR_VERSION_C     3
/* @violates @ref Dio_Siul2_C_REF_3 MISRA 2004 Required Rule 1.4, The used compilers use more than 31 chars for identifiers. */
#define DIO_SIUL2_AR_RELEASE_REVISION_VERSION_C  1
#define DIO_SIUL2_SW_MAJOR_VERSION_C             1
#define DIO_SIUL2_SW_MINOR_VERSION_C             0
#define DIO_SIUL2_SW_PATCH_VERSION_C             1
...
#define SIUL2_GPDI_MASK_U8                  ((uint8)0x1U)

#if ((STD_ON == DIO_REVERSEPORTBITS) || (LOW_BYTE_FIRST == CPU_BYTE_ORDER) || (STD_ON == DIO_READZERO_UNDEFINEDPORTS))
    #define SIUL2_REVERSE_BITS_SHIFT            ((Dio_PortLevelType)1U)
    #define SIUL2_REVERSE_BITS_MASK             ((Dio_PortLevelType)0x1U)
    #define SIUL2_REVERSE_BITS_NUMBER           ((Dio_PortLevelType)((uint8)((sizeof(Dio_PortLevelType)) << 0x3U) - 1U))
#endif /* ((STD_ON == DIO_REVERSEPORTBITS) || (LOW_BYTE_FIRST == CPU_BYTE_ORDER) || (STD_ON == DIO_READZERO_UNDEFINEDPORTS)) */
...
)
{
    ...
    ChannelLevel = (Dio_LevelType)((REG_READ8(SIUL2_GPDI_ADDR32(ChannelId))) & SIUL2_GPDI_MASK_U8);
...
)
{
    ...
    PortLevel = REG_READ16(SIUL2_PGPDI_ADDR32((uint32)(pChannelGroupIdPtr->port)));
    #if (STD_ON == DIO_REVERSEPORTBITS)
        PortLevel = (Dio_PortLevelType)((PortLevel & Dio_Siul2_ReverseBits(pChannelGroupIdPtr->mask)) >> (pChannelGroupIdPtr->u8offset));
    #else
        PortLevel = (Dio_PortLevelType)(Dio_Siul2_ReverseBits(PortLevel));
        PortLevel = (Dio_PortLevelType)((PortLevel & (pChannelGroupIdPtr->mask)) >> (pChannelGroupIdPtr->u8offset));
    #endif
#else
    #if (STD_ON == DIO_REVERSEPORTBITS)
        /** @violates @ref Dio_Siul2_C_REF_2 Cast from unsigned long to pointer */
        ...
        /** @violates @ref Dio_Siul2_C_REF_2 Cast from unsigned long to pointer */
        /** @violates @ref Dio_Siul2_C_REF_6 A cast should not be performed between a pointer type and an integral type. */
        PortLevel = (Dio_PortLevelType)((REG_READ16(SIUL2_PGPDI_ADDR32((uint32)(pChannelGroupIdPtr->port))) & (pChannelGroupIdPtr->mask)) >> (pChannelGroupIdPtr->u8offset));
    #endif
#endif
    return PortLevel;
}
...
#if (LOW_BYTE_FIRST == CPU_BYTE_ORDER)
    u16MpgpdoMppdo = (Dio_PortLevelType)((uint32)Level << pChannelGroupIdPtr->u8offset);
    #if (STD_ON != DIO_REVERSEPORTBITS)
...
#else
    #if (STD_ON == DIO_REVERSEPORTBITS)
        u32Value = ((((uint32)(Dio_Siul2_ReverseBits(Level)) >> (pChannelGroupIdPtr->u8offset)) & (SIUL2_MPGPDO_MPPDO_MASK_U32)) | \
                    (((uint32)(pChannelGroupIdPtr->mask) << (SIUL2_MPGPDO_MASK_SHIFT_U8))) \
                );
    #else
        u32Value = ((((uint32)(Level) << (pChannelGroupIdPtr->u8offset)) & (SIUL2_MPGPDO_MPPDO_MASK_U32)) | \
                    (((uint32)(pChannelGroupIdPtr->mask) << (SIUL2_MPGPDO_MASK_SHIFT_U8))) \
                );
    #endif
#endif
...
#else
    #if (STD_ON == DIO_REVERSEPORTBITS)
        u32Value = (((uint32) Dio_Siul2_ReverseBits(Mask & Dio_aAvailablePinsForWrite[PortId]) << (SIUL2_MPGPDO_MASK_SHIFT_U8)) | (uint32)(Dio_Siul2_ReverseBits(Level)));
    #else
        u32Value = (((uint32) ((uint32)Mask & Dio_aAvailablePinsForWrite[PortId]) << (SIUL2_MPGPDO_MASK_SHIFT_U8)) | (uint32)Level);
    #endif
#endif
